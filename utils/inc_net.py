...
class INCEDERNet(nn.Module):
    def __init__(self, args, pretrained):
        super(INCEDERNet, self).__init__()
        self.convnet_type = args["convnet_type"]
        self.convnets = nn.ModuleList()
        self.pretrained = pretrained
        self.out_dim = None
...
class INCE_FOSTERNet(nn.Module):
    def __init__(self, args, pretrained):
        super(INCE_FOSTERNet, self).__init__()
        self.convnet_type = args["convnet_type"]
        self.convnets = nn.ModuleList()
        self.pretrained = pretrained
        self.out_dim = None
        ...
        # [ADD] InfoNCE 用 projector（必要になったら作る）
        self.projector = None

    # [ADD] projector 初期化（out_dim が確定してから呼ぶ）
    def _init_projector_if_needed(self):
        if self.projector is not None:
            return
        # TagFex と同じキーを流用（無ければデフォルト）
        hid = int(self.args.get("proj_hidden_dim", 512))
        out = int(self.args.get("proj_output_dim", 128))
        self.projector = nn.Sequential(
            TagFex_SimpleLinear(self.out_dim, hid),
            nn.ReLU(inplace=True),
            TagFex_SimpleLinear(hid, out),
        )
    ...
    def forward(self, x):
        ...
        # [ADD] embedding を返す（最後の branch 特徴のみ）
        if self.projector is not None:
            embedding = self.projector(features[:, -self.out_dim:])
            out.update({"embedding": embedding})
    ...
    def update_fc(self, nb_classes):
        ...
        # [ADD] out_dim が確定したので projector を用意
