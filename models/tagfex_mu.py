...
class TagFexMU(BaseLearner):
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs1, inputs2, targets) in enumerate(train_loader):
                ...
                loss = self.lambda_clf * loss_clf + loss_aux + self.lambda_forg * loss_forg
    ...
    def compute_loss(self, inputs, targets):
        ...
        # retain があれば KD を計算
        # TagFex の KD は「predicted_feature を projector に通したもの」と
        # 「前タスク teacher (last_ta_net) の feature を projector に通したもの」の InfoNCE distill
        if mask_retain.any() and (self._cur_task > 0) and hasattr(self, "last_ta_net") and hasattr(self, "last_projector"):

            # predicted_feature / teacher_feature を retain のみ取り出す
            r_pred_feat = predicted_feature[mask_retain]

            # teacher feature は勾配不要
            with torch.no_grad():
                old_out = self.last_ta_net(inputs.contiguous())
                # 実装差異に備えて features の取り出しを頑丈に
                if isinstance(old_out, dict) and ("features" in old_out):
                    old_feat = old_out["features"]
                else:
                    old_feat = old_out
                r_old_feat = old_feat[mask_retain]

            # projector を通す（student側は勾配あり、teacher側は no_grad でOK）
            p_feats = self.last_projector(r_pred_feat)
            with torch.no_grad():
                z_feats = self.last_projector(r_old_feat)

            # InfoNCE distill は 2-view 前提（偶数個）を想定
            if (p_feats.shape[0] >= 2) and (p_feats.shape[0] % 2 == 0):
                loss_kd = infoNCE_distill_loss(
                    p_feats, z_feats, self.args["infonce_kd_temp"]
                )

        #=== trans_cls_loss を計算 ===#
        trans_cls_loss = torch.tensor(0., device=self._device)

        # trans_logits は「current task クラス（increment 分）」に対する logits（想定）
        # current task（targets >= known_classes）かつ retain のみで CE を計算
        if (trans_logits is not None) and mask_retain.any() and (self._cur_task > 0):
            cur_task_mask = (targets >= self._known_classes)
            cur_retain_mask = cur_task_mask & mask_retain

            if cur_retain_mask.any():
                trans_cls_loss = F.cross_entropy(
                    trans_logits[cur_retain_mask],
                    targets[cur_retain_mask] - self._known_classes
                )

        #=== transfer_loss を計算 ===#
        transfer_loss = torch.tensor(0., device=self._device)

        # 論文/実装の流儀：trans_cls_loss が小さい時だけ transfer を掛ける（元TagFexの条件）
        # teacher = trans_logits（detach）、student = logits の new-class 部分を KL で合わせる
        if (trans_logits is not None) and mask_retain.any() and (self._cur_task > 0):
            cur_task_mask = (targets >= self._known_classes)
            cur_retain_mask = cur_task_mask & mask_retain

            if cur_retain_mask.any():
                if trans_cls_loss.item() < loss_clf.item():
                    T = self.args.get("kd_temp", 1.0)
                    transfer_loss = F.kl_div(
                        F.log_softmax(logits[cur_retain_mask][:, self._known_classes:] / T, dim=1),
                        F.softmax(trans_logits.detach()[cur_retain_mask] / T, dim=1),
                        reduction="batchmean",
                    )
        ...
        auto_kd_factor = float(self._known_classes) / float(self._total_classes) if self._total_classes > 0 else 0.0

        # contrastive 部分は retain のみ（infonce_loss / loss_kd は retain 条件で 0 のままになっている）
        loss_contrast = (
            infonce_loss * (1.0 - auto_kd_factor)
            + self.args.get("contrast_kd_factor", 1.0) * loss_kd * auto_kd_factor
        )

        loss_aux_total = (
            self.args.get("aux_factor", 1.0) * loss_aux
            + self.args.get("contrast_factor", 1.0) * loss_contrast
            + self.args.get("trans_cls_factor", 1.0) * trans_cls_loss
            + self.args.get("transfer_factor", 1.0) * transfer_loss
        )

        return logits, loss_clf, loss_aux_total, loss_forg
